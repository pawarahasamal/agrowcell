...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 border-b border-white/10 z-50 bg-white/10 backdrop-blur-sm text-white">
      <div className="container mx-auto px-4 py-2 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden lg:flex items-center space-x-4">
            ...
            {Object.keys(menuItems).map((key) => (
              <div key={key} className="relative">
                ...
                {activeDropdown === key && (
                  <div className="absolute left-0 mt-2 w-screen max-w-md bg-white/10 backdrop-blur-sm rounded-md shadow-lg py-1 text-white z-50">
                    <div className="grid md:grid-cols-2 grid-cols-1 gap-4 p-4">
                      {menuItems[key].items.map((item, idx) => (
                        <Link
                          key={idx}
